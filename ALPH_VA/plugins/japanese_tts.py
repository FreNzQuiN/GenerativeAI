...
from voicevox.errors import HttpException, NotfoundError
...
if not os.path.exists(VOICEVOX_AUDIO_DIR):
    try:
        ...
        VOICEVOX_AUDIO_DIR = _base_audio_path
...
async def generate_speech(text: str, speaker_id: int | None = None) -> str | None:
    ...
    try:
        ...
        return None
    except (HttpException, NotfoundError) as e_vv_sdk:
...
def remove_all_voicevox_outputs():
    removed_count = 0
    if not os.path.isdir(VOICEVOX_AUDIO_DIR):
        logger.warning(
